  ...
  return (
    <group position={pos}>
      <mesh position={[0, H / 2 + 0.04, 0]}>
        <boxGeometry args={[W + 0.16, 0.06, 0.06]} />
        <meshStandardMaterial color={bCol} roughness={0.25} metalness={0.6} />
      </mesh>
      <mesh position={[0, -H / 2 - 0.06, 0]}>
        <boxGeometry args={[W + 0.16, 0.1, 0.06]} />
        <meshStandardMaterial color={bCol} roughness={0.25} metalness={0.6} />
      </mesh>
      ...
      <mesh position={[0, -H / 2 - 0.07, 0.09]}>
        <boxGeometry args={[W - 0.1, 0.02, 0.006]} />
      ...
      <mesh position={[-(W / 2 + 0.04), 0, 0.06]}>
        <boxGeometry args={[0.008, H, 0.006]} />
        <meshStandardMaterial
          color="#ff4400"
          emissive="#ff2200"
      ...
      <mesh position={[0, -H / 2 - 0.14, 0.04]}>
        <boxGeometry args={[0.13, 0.22, 0.1]} />
        <meshStandardMaterial color="#c4c8d8" roughness={0.4} metalness={0.7} />
      </mesh>
...
function MSIMonitor({
  ...
  return (
    <group position={pos} rotation={rot}>
      <mesh>
        <boxGeometry args={[2.26, 1.4, 0.06]} />
        <meshStandardMaterial
          color="#1c1c22"
          roughness={0.4}
          metalness={0.65}
      ...
      <mesh position={[0, 0, 0.035]}>
        <planeGeometry args={[2.08, 1.24]} />
        <ScreenMat color="#f97316" on={on} distort={0.22} />
      </mesh>
      {on && (
        <>
          <Text
      ...
      <mesh position={[0, -0.71, 0.036]}>
        <boxGeometry args={[1.9, 0.02, 0.007]} />
        <meshStandardMaterial
          color="#dc2626"
          emissive="#cc1111"
      ...
      <mesh position={[0, 0, -0.055]}>
        <boxGeometry args={[0.09, 0.12, 0.055]} />
        <meshStandardMaterial color="#222228" roughness={0.5} metalness={0.8} />
      </mesh>
    </group>
  );
}
  ...
  return (
    <group position={pos}>
      ...
      <mesh position={[0, 1.22, 0]}>
        ...
        <meshStandardMaterial color="#1e1e24" roughness={0.5} metalness={0.8} />
      </mesh>
...
function GamingPC({ pos }: { pos: [number, number, number] }) {
  ...
  return (
    <group position={pos}>
      <mesh>
        <boxGeometry args={[0.68, 1.85, 0.8]} />
      ...
      {/* Power LED */}
      <mesh position={[0.342, 0.8, 0.28]} rotation={[0, 0, Math.PI / 2]}>
      ...
      <mesh position={[0, 0.928, 0]}>
        <boxGeometry args={[0.65, 0.012, 0.76]} />
        <meshStandardMaterial
          color="#111118"
          roughness={0.85}
          metalness={0.4}
      ...
      {([-0.22, 0.22] as number[]).flatMap((x) =>
        ([-0.28, 0.28] as number[]).map((z) => (
          <mesh key={`${x}-${z}`} position={[x, -0.932, z]}>
            <boxGeometry args={[0.055, 0.024, 0.055]} />
...
function FalchionLP({ pos }: { pos: [number, number, number] }) {
  const cols = useMemo(
    () => Array.from({ length: 70 }, (_, i) => KC[i % KC.length]),
    [],
  );
  return (
    <group position={pos}>
      <mesh>
        <boxGeometry args={[1.3, 0.032, 0.46]} />
        <meshStandardMaterial
          color="#d4d8e8"
          roughness={0.2}
          metalness={0.55}
      ...
      <mesh position={[0, -0.025, 0]}>
        <boxGeometry args={[1.32, 0.014, 0.475]} />
        <meshStandardMaterial
          color="#c4c8d8"
          roughness={0.3}
          metalness={0.45}
      ...
      {Array.from({ length: 5 }, (_, row) =>
        Array.from({ length: 14 }, (_, col) => {
          ...
          const kw = (isSpace ? 3.5 : 1) * 0.082 - 0.01;
          const cx =
            -0.565 + col * 0.086 + (isSpace ? (col - 4) * 0.082 * 0.5 : 0);
          return (
            <mesh
              key={`${row}-${col}`}
              position={[cx, 0.024, -0.175 + row * 0.086]}
            >
              <boxGeometry args={[kw, 0.015, 0.07]} />
              <meshStandardMaterial
                color={cols[row * 14 + col]}
                emissive={cols[row * 14 + col]}
...
function HarpeAceMini({
  ...
  return (
    <group position={pos} rotation={r}>
      <mesh>
        <boxGeometry args={[0.16, 0.046, 0.282]} />
        <meshStandardMaterial
          color="#e6e8f2"
          roughness={0.2}
          metalness={0.16}
      ...
      <mesh position={[0, 0.026, -0.042]}>
        <boxGeometry args={[0.146, 0.018, 0.188]} />
        <meshStandardMaterial color="#eef0fa" roughness={0.16} />
      </mesh>
      <mesh position={[-0.042, 0.027, -0.038]}>
        <boxGeometry args={[0.062, 0.006, 0.15]} />
        <meshStandardMaterial color="#e0e2ec" roughness={0.27} />
      </mesh>
      <mesh position={[0.042, 0.027, -0.038]}>
        <boxGeometry args={[0.062, 0.006, 0.15]} />
        <meshStandardMaterial color="#e0e2ec" roughness={0.27} />
      </mesh>
      <mesh position={[0, 0.032, -0.054]} rotation={[Math.PI / 2, 0, 0]}>
        <cylinderGeometry args={[0.015, 0.015, 0.048, 14]} />
        <meshStandardMaterial
          color="#b6b8cc"
          roughness={0.55}
          metalness={0.35}
...
function Mousepad({ pos }: { pos: [number, number, number] }) {
  return (
    <mesh position={pos}>
      <boxGeometry args={[1.9, 0.005, 1.0]} />
      <meshStandardMaterial color="#0b0b10" roughness={0.97} />
    </mesh>
  );
}
...
function Desk() {
  return (
    <group>
      <mesh position={[0, -1.56, 0.6]}>
        <boxGeometry args={[8.5, 0.07, 2.9]} />
        <meshStandardMaterial
          color="#1a1510"
          roughness={0.68}
          metalness={0.12}
      ...
      {([-3.8, 3.8] as number[]).flatMap((x) =>
        ([-0.4, 1.7] as number[]).map((z) => (
          <mesh key={`${x}-${z}`} position={[x, -2.84, z]}>
            <boxGeometry args={[0.055, 2.58, 0.055]} />
            <meshStandardMaterial
              color="#18181e"
              roughness={0.5}
              metalness={0.85}
      ...
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -4.26, 0]}>
        <planeGeometry args={[30, 24]} />
        <meshStandardMaterial color="#0c0c0e" roughness={0.98} />
      </mesh>
      <mesh position={[0, 2, -2.3]}>
        <planeGeometry args={[30, 14]} />
        <meshStandardMaterial color="#0e0e12" roughness={1.0} />
      </mesh>
      <mesh rotation={[0, -Math.PI / 2, 0]} position={[6.2, 1, 1.5]}>
        <planeGeometry args={[8, 12]} />
        <meshStandardMaterial color="#0f0f13" roughness={1.0} />
      </mesh>
    </group>
  );
}
...
function Scene() {
  return (
    <>
      ...
      <ambientLight intensity={0.1} color="#1a1030" />
      ...
      <Desk />
      <OdysseyG9 pos={[0, 0.3, 0]} />
      <MonitorArm pos={[0.4, 0.4, -0.4]} />
      <MSIMonitor pos={[0.4, 2.28, -0.54]} rot={[0.3, 0.0, 0]} />
      <GamingPC pos={[3.6, 0.37, 0.9]} />
      <Mousepad pos={[0.65, -1.525, 1.38]} />
      <FalchionLP pos={[0, -1.506, 1.38]} />
      <HarpeAceMini pos={[1.3, -1.507, 1.3]} rot={[0, 0.08, 0]} />
    </>
  );
}
    ...
    return (
      <div
        style={{
          ...
          height: "100%",
          background:
            "radial-gradient(ellipse 70% 60% at 50% 45%, #1a0a35 0%, #07070a 70%)",
          display: "flex",
