...
/**
 * Built lazily so the profile/social values come from the CMS store rather
 * than being baked in at import. The local rebinds keep the entries below
 * unchanged.
 */
const getKB = (): KBEntry[] => {
  const profile = getProfileSnapshot();
  const FULL_NAME = profile.fullName;
  const JOB_TITLE = profile.jobTitle;
  const EMAIL_ADDRESS = profile.email;
  const GITHUB_URL = getSocialUrlSnapshot("github");
  const LINKED_IN_URL = getSocialUrlSnapshot("linkedIn");
  return [
    // Greetings
    {
      keys: [
    ...
    // About the bot
    {
      keys: [
        ...
        "are you bot",
        "what is chatgpm",
        "what does chatgpm mean",
        "what does gpm stand for",
        "generative personal machine",
        "gene paul model",
    ...
    // Technologies most comfortable with
    {
      keys: [
        ...
        "what tech is he comfortable",
        "go to technologies",
        "technologies most comfortable",
      ],
...
export function ChatContent() {
  ...
  useEffect(() => {
    ...
    if (!msg) return;
    if (typewriterLen >= msg.text.length) {
    ...
    >
      ...
      <div className="shrink-0 flex items-center gap-2.5 px-4 py-2.5 border-b border-a26-glass-border bg-a26-title-bar">
        ...
        <div className="flex-1 min-w-0">
          <div className="text-a26-text text-[13px] font-semibold leading-none mb-0.5">
            ChatGPM
          </div>
          <div className="text-a26-muted text-[10px]">
            AI · GPM Knowledge Base
          </div>
        </div>
        <span
      ...
      >
        {messages.map((msg) => {
          ...
          const displayText = isAnimating
            ? msg.text.slice(0, typewriterLen)
            : msg.text;
          return (
            <div
              key={msg.id}
            ...
            >
              ...
              <div
                className={`max-w-[78%] px-3 py-2 text-[12.5px] leading-[1.68] whitespace-pre-wrap wrap-break-word ${
                  msg.role === "user"
                    ? "rounded-[14px] rounded-tr-lg"
                    : "rounded-[14px] rounded-tl-lg"
                }`}
                style={
                  msg.role === "user"
                ...
                {Array.from({ length: totalPages }).map((_, i) => (
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: 3,
                      background:
                        i === suggPage ? "#A855F7" : "rgba(255,255,255,0.18)",
                    }}
                  />
                ))}
              </div>
