...
export function HanoiContent() {
  ...
  const numDiscsRef = useRef(numDiscs);
  numDiscsRef.current = numDiscs;
  ...
  const discWidth = (size: number) =>
    MIN_DISC +
    ((size - 1) / Math.max(numDiscsRef.current - 1, 1)) * (MAX_DISC - MIN_DISC);
  ...
  const pegFromClientX = useCallback((clientX: number): number => {
    ...
    return Math.max(
      0,
      Math.min(2, Math.floor((clientX - rect.left) / (rect.width / 3))),
    );
  }, []);
  ...
  const startDrag = useCallback(
    (e: React.PointerEvent, fromPeg: number, discSize: number) => {
      if (phaseRef.current !== "playing") return;
      e.preventDefault();
  ...
  useEffect(() => {
    ...
    const onMove = (e: PointerEvent) => {
      ...
      setDragging((prev) =>
        prev ? { ...prev, x: e.clientX, y: e.clientY, hovered, moved } : null,
      );
    };
                  ...
                  {Array.from({ length: d.discs }, (_, i) => (
                    <div
                      ...
                      style={{
                        ...
                        height: 7,
                        borderRadius: 2,
                        background: DISC_COLORS[d.discs - 1 - i],
                        opacity: 0.75,
                      }}
                    />
                  ))}
    ...
    >
      ...
      >
        <div
          ...
          style={{ alignItems: "flex-end", paddingBottom: 0 }}
        >
          {[0, 1, 2].map((pegIdx) => {
            ...
            const dropInvalid =
              isDragHov &&
              !canDrop(dragging!.fromPeg, pegIdx) &&
              dragging!.fromPeg !== pegIdx;
            ...
            return (
              <motion.div
                key={pegIdx}
                animate={
                  shake === pegIdx ? { x: [-7, 7, -5, 5, -3, 3, 0] } : { x: 0 }
                }
                transition={{ duration: 0.38 }}
                onClick={() => {
                  if (!dragging) handlePegClick(pegIdx);
                }}
                style={{
                  flex: 1,
              ...
              >
                {/* Peg area */}
                <div
                  style={{ position: "relative", width: "100%", height: PEG_H }}
                >
                  {/* Column background */}
                  <div
                    style={{
                  ...
                  {discs.map((size, i) => {
                    ...
                    return (
                      <div
                        key={size}
                        onPointerDown={
                          isTop ? (e) => startDrag(e, pegIdx, size) : undefined
                        }
                        style={{
                          position: "absolute",
                          ...
                          transition: isGhost
                            ? "none"
                            : "transform 0.18s ease, box-shadow 0.18s",
                          boxShadow: isLifted
                ...
                >
                  ...
                  {!isMobile && (
                    <span
                      style={{ marginLeft: 4, opacity: 0.55, fontWeight: 400 }}
                    >
                      [{pegIdx + 1}]
                    </span>
                  )}
      ...
      {dragging?.moved &&
        (() => {
          const color =
            DISC_COLORS[(dragging.discSize - 1) % DISC_COLORS.length];
