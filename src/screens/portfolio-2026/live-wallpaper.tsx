...
function drawClock(
  ...
  ox: number, oy: number, fs: number, now: Date,
) {
  ...
  ctx.fillText(`${now.getDate()} ${MONTHS[now.getMonth()]} ${now.getFullYear()}`, ox, oy + fs * 0.80);
  ...
  for (let i = 0; i < 12; i++) {
    ...
    ctx.strokeStyle = i % 3 === 0 ? "rgba(255,255,255,0.52)" : "rgba(255,255,255,0.20)";
  ...
  ctx.lineTo(cx + Math.cos(ha) * cr * 0.50, cy + Math.sin(ha) * cr * 0.50);
  ctx.strokeStyle = "rgba(255,255,255,0.90)"; ctx.lineWidth = 2.0; ctx.lineCap = "round"; ctx.stroke();

  const ma = (now.getMinutes() / 60 + now.getSeconds() / 3600) * Math.PI * 2 - Math.PI / 2;
  ctx.beginPath(); ctx.moveTo(cx, cy);
  ctx.lineTo(cx + Math.cos(ma) * cr * 0.72, cy + Math.sin(ma) * cr * 0.72);
  ctx.strokeStyle = "rgba(255,255,255,0.90)"; ctx.lineWidth = 1.3; ctx.stroke();

  const sa = (now.getSeconds() / 60 + now.getMilliseconds() / 60000) * Math.PI * 2 - Math.PI / 2;
  ctx.beginPath(); ctx.moveTo(cx, cy);
  ctx.lineTo(cx + Math.cos(sa) * cr * 0.80, cy + Math.sin(sa) * cr * 0.80);
  ctx.strokeStyle = "rgba(125,192,26,0.88)"; ctx.lineWidth = 0.9; ctx.stroke();
  ...
  ctx.fillText(`${hh}:${mm}`, cx, cy + cr + fs * 0.70);
  ctx.textAlign = "left";
  ctx.restore();
}

// ── Helpers ────────────────────────────────────────────────────────────────────
function makeDrop(W: number, H: number, layer: 0 | 1 | 2, randomY = true): Drop {
  // layer 0 = far/background  layer 1 = mid  layer 2 = near/foreground
  const configs = [
    { lenMin: 8,  lenMax: 15, spdMin: 5,  spdMax: 9,  wMin: 0.28, wMax: 0.45, opMin: 0.05, opMax: 0.11 },
    { lenMin: 15, lenMax: 28, spdMin: 10, spdMax: 15, wMin: 0.45, wMax: 0.68, opMin: 0.14, opMax: 0.24 },
    { lenMin: 28, lenMax: 52, spdMin: 17, spdMax: 28, wMin: 0.60, wMax: 1.05, opMin: 0.22, opMax: 0.42 },
  ] as const;
  const c = configs[layer];
  const spd = c.spdMin + Math.random() * (c.spdMax - c.spdMin);
  return {
    x:   Math.random() * (W + 100) - 50,
    y:   randomY ? Math.random() * H : -(8 + Math.random() * H * 0.30),
...
function CanvasOverlay() {
  ...
  useEffect(() => {
    ...
    const init = (W: number, H: number) => {
      ...
      const WARM = [
        [255,175,80],[255,200,105],[255,220,140],[255,155,60],
        [255,215,125],[200,225,255],[220,200,255],
      ] as const;
    ...
    const clockId = setInterval(() => { clockRef.current = new Date(); }, 1000);
    ...
    function drawDrop(d: Drop) {
      ...
      g.addColorStop(0.60, `rgba(210,228,255,${d.op})`);
      g.addColorStop(0.88, `rgba(210,228,255,${d.op * 0.80})`);
      g.addColorStop(1,    `rgba(210,228,255,${d.op * 0.20})`);
    ...
    const draw = (now: number) => {
      ...
      bokehs.current.forEach((b) => {
        const p = 0.80 + 0.20 * Math.sin(t * 0.80 + b.phase);
        const [r, g, bl] = b.rgb;
      ...
      fg.current.forEach((d, i) => {
        ...
        if (advanceDrop(d, dt, H)) {
          ...
          if (d.y > H * 0.72 && splashes.current.length < 60 && Math.random() < 0.55) {
      ...
      splashes.current = splashes.current.filter((s) => {
        ...
        s.r   = s.maxR * progress;
        s.op  = (0.55 + 0.30 * Math.random()) * (1 - progress);  // fade out
        s.age += dt;
    ...
    raf = requestAnimationFrame((n) => { prev = n; draw(n); });
