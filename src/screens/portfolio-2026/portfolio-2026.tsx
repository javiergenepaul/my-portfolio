...
import {
  FolderOpen, Settings2, TerminalSquare,
  ...
  FileText, X, Minus, Maximize2,
  Sun, Moon, Check, ZoomIn, ZoomOut, Download, Sparkles, LayoutTemplate,
} from "lucide-react";
import {
...
const ResumeSimple = dynamic(
  () => import("@/screens/resume/templates/simple-template").then(m => ({ default: m.SimpleTemplate })),
  { ssr: false, loading: () => <div style={{ width: 794, height: 600, background: "#f5f5f5" }} /> },
);
const ResumeModern = dynamic(
  () => import("@/screens/resume/templates/modern-template").then(m => ({ default: m.ModernTemplate })),
  { ssr: false, loading: () => <div style={{ width: 794, height: 600, background: "#1a1a1a" }} /> },
);
...
const MAC_FONT = "-apple-system, BlinkMacSystemFont, 'SF Pro Display', 'Helvetica Neue', 'Segoe UI', system-ui, sans-serif";
...
type WinId = "about" | "projects" | "terminal" | "skills" | "contact" | "resume";
...
const WIN_DEFS: WinDef[] = [
  { id: "about",    title: "About Me",       color: A.teal,    icon: <User size={14} />,           defaultPos: { x: 80,  y: 50 }, defaultSize: { w: 720, h: 580 } },
  ...
  { id: "contact",  title: "Contact",         color: "#FB7185", icon: <Mail size={14} />,           defaultPos: { x: 160, y: 80 }, defaultSize: { w: 480, h: 480 } },
  { id: "resume",   title: "Resume Builder",  color: "#F59E0B", icon: <FileText size={14} />,       defaultPos: { x: 180, y: 85 }, defaultSize: { w: 1060, h: 760 } },
];
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 2, flex: 1 }}>
        <div style={{ padding: "0 8px", height: 28, display: "flex", alignItems: "center" }}>
        ...
        <span style={{ fontSize: 13, fontWeight: 600, color: A.text, padding: "0 8px" }}>Gene Paul</span>
        {["File", "View", "Go", "Window"].map((item) => (
          <button key={item}
            ...
            style={{
              ...
              background: activeMenu === item ? "rgba(255,255,255,0.10)" : "transparent",
              borderRadius: 4, border: "none",
              color: activeMenu === item ? A.text : A.textMid,
              fontSize: 13, cursor: "default", fontFamily: MAC_FONT,
            }}>
            {item}
          </button>
        ))}
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
        <button onClick={onCmdK}
          style={{
            ...
            borderRadius: 5, color: A.textMid, fontSize: 11,
            cursor: "pointer", fontFamily: MAC_FONT,
          }}
          title="Command Palette (⌘K)"
          aria-label="Open command palette"
        >
          <Search size={10} /><Command size={9} /><span style={{ fontSize: 10 }}>K</span>
        </button>
        <span style={{ fontSize: 12, color: A.text, fontWeight: 400 }}>{time}</span>
      </div>
    </div>
  );
}
...
function TrafficLights({ onClose, onMinimize, onMaximize }: {
  ...
  return (
    <div style={{ display: "flex", gap: 8, alignItems: "center" }}
      ...
      {[
        { bg: "#FF5F57", label: "Close",    onClick: onClose,    icon: <X size={7} color="rgba(0,0,0,0.55)" strokeWidth={2.5} /> },
        { bg: "#FFBD2E", label: "Minimize", onClick: onMinimize, icon: <Minus size={7} color="rgba(0,0,0,0.55)" strokeWidth={2.5} /> },
        { bg: "#28C840", label: "Maximize", onClick: onMaximize, icon: <Maximize2 size={7} color="rgba(0,0,0,0.55)" strokeWidth={2.5} /> },
      ].map(btn => (
        <button key={btn.label} onClick={btn.onClick} aria-label={btn.label}
          style={{ width: 12, height: 12, borderRadius: "50%", background: btn.bg, border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
          {hov && btn.icon}
        </button>
      ))}
    </div>
...
function AboutContent() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", fontFamily: MAC_FONT }}>
      {/* Safari-style address bar */}
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 12px", borderBottom: `1px solid ${A.glassBorder}`, background: A.titleBar, flexShrink: 0 }}>
        <ChevronLeft size={13} color={A.textMuted} />
        <ChevronRight size={13} color={A.textMuted} />
        <RefreshCw size={12} color={A.textMuted} />
        <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 5, background: A.inputBg, borderRadius: 7, padding: "3px 10px", border: `1px solid ${A.glassBorder}`, fontSize: 12, color: A.textMid }}>
          <Lock size={10} color={A.teal} /> genepaulmarjavier.dev/2026/about
        </div>
      </div>
      {/* Tabs */}
      <div style={{ display: "flex", gap: 2, padding: "6px 14px 0", borderBottom: `1px solid ${A.glassBorder}`, flexShrink: 0 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 22 }}>
                <div style={{ display: "flex", gap: 20, alignItems: "center" }}>
                  <div style={{ position: "relative", flexShrink: 0 }}>
                    ...
                    <div style={{ position: "absolute", bottom: 2, right: 2, width: 12, height: 12, borderRadius: "50%", background: A.green, border: "2px solid #1C1C1C" }} />
                  </div>
                  <div>
                    <h1 style={{ margin: 0, fontSize: 24, fontWeight: 700, color: A.text }}>{FULL_NAME}</h1>
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  {[
                    ...
                    <a key={l.label} href={l.href} target="_blank" rel="noopener noreferrer"
                      style={{ display: "flex", alignItems: "center", gap: 6, background: A.glass, border: `1px solid ${A.glassBorder}`, borderRadius: 7, padding: "6px 12px", fontSize: 12, color: l.c, textDecoration: "none" }}>
                      {l.icon} {l.label}
                    </a>
                  ))}
                </div>
...
function ProjectsContent() {
  ...
  return (
    <div style={{ display: "flex", height: "100%", fontFamily: MAC_FONT }}>
      <div style={{ width: 155, flexShrink: 0, background: A.sidebar, borderRight: `1px solid ${A.glassBorder}`, padding: "12px 6px", display: "flex", flexDirection: "column", gap: 1 }}>
        ...
        {sidebar.map((s) => (
          <button key={s.id} onClick={() => setFilter(s.id)}
            style={{ display: "flex", alignItems: "center", gap: 7, width: "100%", padding: "6px 9px", borderRadius: 6, border: "none", background: filter === s.id ? `rgba(${hexRgb(A.blue)},0.14)` : "transparent", color: filter === s.id ? A.blue : A.textMid, fontSize: 12, cursor: "pointer", textAlign: "left", fontFamily: MAC_FONT }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "14px 16px", scrollbarWidth: "none" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
          {filtered.map((p) => (
            <div key={p.projectId}
              style={{ background: A.card, border: `1px solid ${A.cardBorder}`, borderRadius: 9, padding: "11px 13px", cursor: "default", transition: "border-color 0.15s" }}
              onMouseEnter={(e) => (e.currentTarget.style.borderColor = `rgba(${hexRgb(A.blue)},0.30)`)}
            ...
            >
              ...
              {(p.previewUrl || p.codeUrl) && (
                <a href={p.previewUrl || p.codeUrl} target="_blank" rel="noopener noreferrer"
                  style={{ display: "inline-flex", alignItems: "center", gap: 3, marginTop: 6, fontSize: 10, color: A.teal, textDecoration: "none" }}>
...
const CMDS: Record<string, () => string[]> = {
  help:   () => ["Available commands:", "  whoami           — personal info", "  ls               — list projects", "  cat skills.json  — print tech stack", "  open <app>       — open About/Projects/Skills/Resume", "  clear            — clear terminal", ""],
  ...
  "cat skills.json": () => {
    ...
    SKILL_CATEGORIES.forEach((c, i) => {
      const n = c.stacks.slice(0, 5).map(s => `"${translate(`services.stack.${s.name}` as any) || s.name}"`).join(", ");
      lines.push(`  "${c.label}": [${n}${c.stacks.length > 5 ? ", ..." : ""}]${i < SKILL_CATEGORIES.length - 1 ? "," : ""}`);
    });
    return [...lines, "}", ""];
  },
};
...
function TerminalContent({ onOpen }: { onOpen: (id: WinId) => void }) {
  ...
  const run = useCallback((cmd: string) => {
    ...
    const appMap: Record<string, WinId> = { "open about": "about", "open projects": "projects", "open skills": "skills", "open contact": "contact", "open resume": "resume" };
    if (appMap[t]) {
      onOpen(appMap[t]);
      setHistory(h => [...h, { input: cmd, output: [`Opening ${appMap[t]}...`, ""] }]);
      setCmdHist(h => [cmd, ...h]); setHistIdx(-1); return;
    }
  ...
  return (
    <div onClick={() => inputRef.current?.focus()}
      style={{ display: "flex", flexDirection: "column", height: "100%", background: A.termBg, fontFamily: "'JetBrains Mono','Fira Code','Cascadia Code',monospace", fontSize: 13, cursor: "text" }}>
...
function SkillsContent() {
  ...
  return (
    <div style={{ display: "flex", height: "100%", fontFamily: MAC_FONT }}>
      <div style={{ width: 170, flexShrink: 0, background: A.sidebar, borderRight: `1px solid ${A.glassBorder}`, padding: "12px 6px", overflowY: "auto", scrollbarWidth: "none" }}>
        ...
        {SKILL_CATEGORIES.map((c, i) => {
          ...
          return (
            <button key={c.key} onClick={() => setActive(c.key)}
              style={{ display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%", padding: "7px 9px", borderRadius: 6, border: "none", background: active === c.key ? `rgba(${hexRgb(col)},0.12)` : "transparent", color: active === c.key ? col : A.textMid, fontSize: 12, cursor: "pointer", fontFamily: MAC_FONT }}>
              <span>{c.label}</span>
            ...
            <motion.div key={active} initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.15 }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
                {cat.stacks.map((s) => {
                  ...
                  return (
                    <div key={s.name}
                      style={{ display: "flex", alignItems: "center", gap: 6, background: A.card, border: `1px solid ${A.cardBorder}`, borderRadius: 7, padding: "5px 11px", fontSize: 12, color: A.text, transition: "border-color 0.14s" }}
...
function ContactContent() {
  ...
  return (
    <div style={{ padding: "28px 32px", display: "flex", flexDirection: "column", gap: 14, fontFamily: MAC_FONT }}>
      ...
      {links.map((l) => (
        <a key={l.label} href={l.href} target="_blank" rel="noopener noreferrer"
          style={{ display: "flex", alignItems: "center", gap: 14, background: A.card, border: `1px solid ${A.cardBorder}`, borderRadius: 10, padding: "14px 18px", textDecoration: "none", transition: "border-color 0.15s" }}
...
function ResumeContent() {
  ...
  useLayoutEffect(() => {
    ...
    if (w > 0) setZoom(Math.min(0.85, Math.max(0.3, Math.floor((w / 794) * 100) / 100)));
  }, []);

  const colors    = RESUME_COLORS[color];
  const stepZoom  = (d: number) => setZoom(z => Math.min(1.2, Math.max(0.3, Math.round((z + d) * 10) / 10)));
  const resetZoom = () => {
    if (!containerRef.current) return;
    const w = containerRef.current.clientWidth - 48;
  ...
  const sectionLabel: React.CSSProperties = {
    ...
    letterSpacing: "0.10em", color: A.textMuted,
            ...
            {([
              ...
              <button key={opt.v} onClick={() => setMode(opt.v)}
                style={{
                  ...
                  background: mode === opt.v ? `rgba(${hexRgb(A.teal)},0.08)` : A.glass,
                  transition: "all 0.14s", fontFamily: MAC_FONT, textAlign: "left",
                }}>
                <span style={{ color: mode === opt.v ? A.teal : A.textMuted }}>{opt.icon}</span>
                <div>
                  <div style={{ fontSize: 12, fontWeight: 600, color: mode === opt.v ? A.text : A.textMid }}>{opt.label}</div>
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            {([
              ...
              <button key={String(opt.v)} onClick={() => setIsDark(opt.v)}
                style={{
                  ...
                  background: isDark === opt.v ? `rgba(${hexRgb(A.blue)},0.08)` : A.glass,
                  transition: "all 0.14s", fontFamily: MAC_FONT,
                }}>
                <span style={{ color: isDark === opt.v ? A.blue : A.textMuted }}>{opt.icon}</span>
                <span style={{ fontSize: 11, color: isDark === opt.v ? A.text : A.textMuted }}>{opt.label}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {RESUME_SWATCHES.map(s => (
              <button key={s.value} onClick={() => setColor(s.value)} title={s.label}
                style={{
                  width: 24, height: 24, borderRadius: "50%", border: "none",
                  background: s.hex, cursor: "pointer", outline: "none",
                  boxShadow: color === s.value ? `0 0 0 2px #1C1C1C, 0 0 0 3.5px ${s.hex}` : "none",
          ...
          <div style={{ fontSize: 11, color: A.textMuted, marginTop: 7, textTransform: "capitalize" }}>{color}</div>
        </div>
        ...
        <div style={{ marginTop: "auto", display: "flex", flexDirection: "column", gap: 10 }}>
          <button onClick={handleExport}
            style={{
              ...
              fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: MAC_FONT,
            }}>
            <Download size={14} /> Export PDF
          </button>
          <div style={{ fontSize: 10, color: A.textMuted, lineHeight: 1.65, padding: "8px 10px", background: A.glass, borderRadius: 7, border: `1px solid ${A.glassBorder}` }}>
            <div style={{ fontWeight: 600, color: A.textMid, marginBottom: 3 }}>Tips</div>
            Select <b style={{ color: A.text }}>Save as PDF</b>, margins → <b style={{ color: A.text }}>None</b>, enable <b style={{ color: A.text }}>Background graphics</b>.
          </div>
        </div>
      </div>

      {/* ── Preview panel ── */}
      <div ref={containerRef} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", background: "rgba(0,0,0,0.18)" }}>

        {/* Zoom toolbar */}
        <div style={{ flexShrink: 0, display: "flex", alignItems: "center", gap: 2, padding: "6px 12px", borderBottom: `1px solid ${A.glassBorder}`, background: A.titleBar }}>
          <button onClick={() => stepZoom(-0.1)}
            style={{ width: 26, height: 22, borderRadius: 5, border: "none", background: "transparent", color: A.textMuted, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <ZoomOut size={13} />
          </button>
          <button onClick={resetZoom}
            style={{ padding: "0 6px", height: 22, borderRadius: 5, border: "none", background: "transparent", color: A.textMid, fontSize: 11, fontFamily: "monospace", cursor: "pointer" }}>
            {Math.round(zoom * 100)}%
          </button>
          <button onClick={() => stepZoom(0.1)}
            style={{ width: 26, height: 22, borderRadius: 5, border: "none", background: "transparent", color: A.textMuted, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        {/* Scrollable resume */}
        <div style={{ flex: 1, overflowY: "auto", overflowX: "auto", padding: "20px", scrollbarWidth: "thin", scrollbarColor: `${A.glassBorder} transparent` }}>
          <div style={{ display: "flex", justifyContent: "center" }}>
            <div id="resume-preview-2026"
              style={{
                width: 794,
...
function AppWindow({
  ...
  return (
    <motion.div
      ...
      exit={{ scale: 0.90, opacity: 0, transition: { duration: 0.16 } }}
      onClick={onFocus}
    >
      {/* Resize handles — invisible hit areas on all 8 edges/corners */}
      {!state.maximized && RESIZE_HANDLES.map(({ edge, cursor, style }) => (
        <div
          key={edge}
      ...
      {/* Title bar */}
      <motion.div onPan={handlePan} onDoubleClick={onMaximize}
        style={{
        ...
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
          ...
          <span style={{ fontSize: 13, fontWeight: 500, color: A.textMid, letterSpacing: "0.01em" }}>{def.title}</span>
        </div>
      </motion.div>
...
function DesktopIcon({ label, icon, color, isOpen, onClick }: {
  label: string; icon: React.ReactNode; color: string; isOpen: boolean; onClick: () => void;
}) {
  const [hov, setHov] = useState(false);
  return (
    <button onClick={onClick}
      ...
      style={{
        ...
        transition: "background 0.12s", fontFamily: MAC_FONT,
      }}
    ...
    >
      ...
      <span style={{ fontSize: 11, color: A.text, fontWeight: 500, textShadow: "0 1px 4px rgba(0,0,0,0.95)", background: "rgba(0,0,0,0.40)", borderRadius: 4, padding: "1px 5px" }}>
        {label}
      </span>
    </button>
  );
...
function Dock({ windows, onOpen, onRestore }: {
  ...
  return (
    <div style={{
      ...
      {dockApps.map((app) => {
        ...
        return (
          <div key={app.id} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4, position: "relative" }}>
            {/* macOS hover label */}
            <AnimatePresence>
              {isHov && (
                <motion.div
                  ...
                  style={{
                    ...
                    fontSize: 12, color: A.text, whiteSpace: "nowrap",
            ...
            <motion.button
              ...
              style={{
                ...
                cursor: "pointer",
                boxShadow: app.isOpen ? `0 0 18px rgba(${hexRgb(app.color)},0.22)` : "none",
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4, position: "relative" }}>
        <AnimatePresence>
          {hov === "gh" && (
            <motion.div initial={{ opacity: 0, y: 4, scale: 0.90 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.11 }}
              style={{ position: "absolute", bottom: "calc(100% + 10px)", background: "rgba(28,28,28,0.94)", border: "1px solid rgba(255,255,255,0.10)", borderRadius: 7, padding: "4px 9px", fontSize: 12, color: A.text, whiteSpace: "nowrap", backdropFilter: "blur(12px)", pointerEvents: "none", boxShadow: "0 4px 14px rgba(0,0,0,0.50)" }}>
              GitHub
            </motion.div>
          )}
        </AnimatePresence>
        <motion.a animate={{ width: 44, height: 44 }}
      ...
      {open && (
        <>
          <motion.div key="ov" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose}
            style={{ position: "fixed", inset: 0, zIndex: 9500, background: "rgba(0,0,0,0.55)", backdropFilter: "blur(4px)" }} />
          <motion.div key="pl"
            initial={{ opacity: 0, scale: 0.96, y: -14 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.17 }}
            style={{ position: "fixed", top: "18%", left: "50%", transform: "translateX(-50%)", width: "min(540px,calc(100vw-32px))", background: "rgba(18,18,18,0.97)", border: `1px solid ${A.windowBorder}`, borderRadius: 13, overflow: "hidden", zIndex: 9501, boxShadow: "0 28px 64px rgba(0,0,0,0.78)", fontFamily: MAC_FONT }}>
            <div style={{ display: "flex", alignItems: "center", gap: 9, padding: "11px 14px", borderBottom: `1px solid ${A.glassBorder}` }}>
              ...
              <input ref={inputRef} value={q} onChange={e => setQ(e.target.value)} onKeyDown={e => e.key === "Escape" && onClose()} placeholder="Search commands..."
                style={{ flex: 1, background: "none", border: "none", outline: "none", color: A.text, fontSize: 14, fontFamily: MAC_FONT }}
            ...
            <div style={{ padding: "5px 6px 6px", maxHeight: 300, overflowY: "auto", scrollbarWidth: "none" }}>
              {filtered.map((item, i) => (
                <button key={i} onClick={() => { item.action(); onClose(); }}
                  style={{ display: "flex", alignItems: "center", gap: 9, width: "100%", padding: "8px 9px", borderRadius: 7, border: "none", background: "transparent", color: A.text, fontSize: 13, cursor: "pointer", textAlign: "left", transition: "background 0.1s", fontFamily: MAC_FONT }}
...
function MobilePortfolio() {
  ...
  const mobileApps: { id: WinId; label: string; icon: React.ReactNode; color: string }[] = [
    { id: "about",    label: "About",    icon: <User size={26} />,           color: A.teal    },
    { id: "projects", label: "Projects", icon: <FolderOpen size={26} />,     color: A.blue    },
    { id: "terminal", label: "Terminal", icon: <TerminalSquare size={26} />, color: A.green   },
    { id: "skills",   label: "Skills",   icon: <Settings2 size={26} />,      color: A.violet  },
    { id: "contact",  label: "Contact",  icon: <Mail size={26} />,           color: "#FB7185" },
    { id: "resume",   label: "Resume",   icon: <FileText size={26} />,       color: "#F59E0B" },
  ];
  ...
  return (
    ...
    <div style={{ position: "relative", width: "100vw", height: "100dvh", overflow: "hidden", fontFamily: MAC_FONT }}>
      ...
      <div style={{ position: "absolute", inset: 0, zIndex: 2, display: "flex", flexDirection: "column" }}>
        ...
        {/* Status bar */}
        <div style={{ height: 48, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 22px", background: "rgba(0,0,0,0.18)", backdropFilter: "blur(10px)", WebkitBackdropFilter: "blur(10px)" }}>
          <span style={{ fontSize: 15, fontWeight: 600, color: A.text, minWidth: 60 }}>{time}</span>
          {/* Signal + WiFi + Battery */}
          <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
            {/* Cell bars */}
            <div style={{ display: "flex", alignItems: "flex-end", gap: 2 }}>
              {[7, 10, 13, 16].map((h, i) => (
                <div key={i} style={{ width: 3, height: h, borderRadius: 1.5, background: i < 3 ? A.text : "rgba(255,255,255,0.3)" }} />
              ))}
            </div>
            {/* WiFi arcs */}
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 1 }}>
              <div style={{ width: 22, height: 12, borderRadius: 3, border: `1.5px solid rgba(255,255,255,0.7)`, padding: 1.5, display: "flex", alignItems: "stretch" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", overflowX: "hidden", padding: "28px 24px 110px", scrollbarWidth: "none" }}>

          {/* Profile header */}
          <div style={{ textAlign: "center", marginBottom: 32 }}>
            <div style={{ width: 72, height: 72, borderRadius: "50%", overflow: "hidden", border: `2.5px solid ${A.teal}`, boxShadow: `0 0 24px rgba(${hexRgb(A.teal)},0.35)`, margin: "0 auto 12px" }}>
              <Image src={AvatarProfile} alt={FULL_NAME} width={72} height={72} style={{ objectFit: "cover", width: "100%", height: "100%" }} />
            </div>
            <h1 style={{ margin: 0, fontSize: 20, fontWeight: 700, color: A.text, letterSpacing: "-0.01em" }}>{FULL_NAME}</h1>
            <p style={{ margin: "4px 0 0", fontSize: 13, color: A.teal, fontWeight: 500 }}>{JOB_TITLE}</p>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 18, maxWidth: 340, margin: "0 auto 32px" }}>
            {mobileApps.map(app => (
              <motion.button key={app.id} whileTap={{ scale: 0.86 }} onClick={() => setActiveApp(app.id)}
                style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, background: "none", border: "none", cursor: "pointer", padding: 4, fontFamily: MAC_FONT }}>
                <div style={{
                ...
                <span style={{ fontSize: 11, color: A.text, fontWeight: 500, textShadow: "0 1px 4px rgba(0,0,0,0.9)" }}>
                  {app.label}
                </span>
              </motion.button>
            ))}
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: 12 }}>
            {[
              { href: GITHUB_URL,    icon: <Github size={17} />,   color: A.text,    label: "GitHub"   },
              { href: LINKED_IN_URL, icon: <Linkedin size={17} />, color: "#60A5FA", label: "LinkedIn" },
              { href: `mailto:${EMAIL_ADDRESS}`, icon: <Mail size={17} />, color: A.teal, label: "Email" },
            ].map(l => (
              <a key={l.label} href={l.href} target="_blank" rel="noopener noreferrer"
                style={{
                  display: "flex", flexDirection: "column", alignItems: "center", gap: 5,
                  width: 68, padding: "10px 0",
                  background: A.glass, border: `1px solid ${A.glassBorder}`,
                  borderRadius: 16, color: l.color, textDecoration: "none",
                }}>
                {l.icon}
        ...
        <div style={{
          ...
          {dockApps.map(app => (
            <motion.button key={app.id} whileTap={{ scale: 0.86 }}
              ...
              style={{
                ...
                display: "flex", alignItems: "center", justifyContent: "center",
                color: app.color, cursor: "pointer",
        ...
        {activeApp && (
          <motion.div
            ...
            <div style={{
              ...
              <button onClick={() => setActiveApp(null)}
                style={{
                  ...
                  color: activeDef?.color ?? A.teal, fontSize: 15,
                  fontFamily: MAC_FONT, padding: "4px 0", minWidth: 70,
                }}>
                <ChevronLeft size={20} />
                <span>Home</span>
              </button>
              <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
                <span style={{ color: activeDef?.color ?? A.teal, opacity: 0.9, display: "flex" }}>{activeDef?.icon}</span>
                <span style={{ fontSize: 14, fontWeight: 600, color: A.text }}>{activeDef?.title}</span>
              </div>
            ...
            <div style={{ flex: 1, overflow: "hidden" }}>
              ...
              {activeApp === "terminal" && <TerminalContent onOpen={(id) => setActiveApp(id)} />}
...
export function Portfolio2026() {
  ...
  return (
    <>
      <a href="#desktop"
        style={{ position: "fixed", top: -40, left: 16, zIndex: 99999, background: A.teal, color: "#05090E", borderRadius: 6, padding: "5px 12px", fontSize: 13, fontWeight: 600, textDecoration: "none", transition: "top 0.15s", fontFamily: MAC_FONT }}
      ...
      <div style={{ position: "relative", zIndex: 1, width: "100vw", height: "100dvh", overflow: "hidden", fontFamily: MAC_FONT }}>
