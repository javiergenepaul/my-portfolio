...
const pageAnim = {
  ...
  exit:    { opacity: 0, y: -10, filter: "blur(3px)", transition: { duration: 0.22, ease: "easeIn" as const } },
};
...
const LEVEL_PCT: Record<string, number> = {
  Native: 100, Fluent: 90, Conversational: 65, Basic: 25,
};

function makeTypeColors(C: Palette): Record<string, { bg: string; color: string; border: string }> {
  return {
    confidential: { bg: "rgba(239,68,68,0.08)",  color: "#DC2626",    border: "rgba(239,68,68,0.2)"   },
    client:       { bg: C.indigoLight,            color: C.indigoDark, border: "rgba(225,29,72,0.2)"   },
    personal:     { bg: C.mintLight,              color: C.mintDark,   border: "rgba(190,18,60,0.2)"   },
    tutorial:     { bg: C.amberLight,             color: "#B45309",    border: "rgba(251,191,36,0.2)"  },
  };
}

function makeThemeColors(C: Palette): Record<string, { bg: string; color: string; border: string }> {
  return {
    Purpose:      { bg: C.indigoLight,            color: C.indigoDark, border: "rgba(225,29,72,0.2)"   },
    Humility:     { bg: C.mintLight,              color: C.mintDark,   border: "rgba(190,18,60,0.2)"   },
    Systems:      { bg: C.amberLight,             color: "#B45309",    border: "rgba(251,191,36,0.2)"  },
    Resilience:   { bg: "rgba(99,102,241,0.08)",  color: "#4F46E5",    border: "rgba(99,102,241,0.2)"  },
    Stoicism:     { bg: "rgba(20,184,166,0.08)",  color: "#0D9488",    border: "rgba(20,184,166,0.2)"  },
    Focus:        { bg: "rgba(168,85,247,0.08)",  color: "#9333EA",    border: "rgba(168,85,247,0.2)"  },
    Leadership:   { bg: "rgba(249,115,22,0.08)",  color: "#EA580C",    border: "rgba(249,115,22,0.2)"  },
    Productivity: { bg: "rgba(16,185,129,0.08)",  color: "#059669",    border: "rgba(16,185,129,0.2)"  },
  };
}

type SkillTab = "backend" | "frontend" | "others";
// accent / glow are constant accent colours that don't change with theme
const SKILL_TABS: { id: SkillTab; label: string; accent: string; glow: string }[] = [
  { id: "backend",  label: "Backend",  accent: "#E11D48", glow: "rgba(225,29,72,0.2)"  },
  { id: "frontend", label: "Frontend", accent: "#BE123C", glow: "rgba(190,18,60,0.18)" },
  { id: "others",   label: "Others",   accent: "#B45309", glow: "rgba(180,83,9,0.18)"  },
];
...
function SidebarPanel({ active, goto }: { active: string; goto: (id: string) => void }) {
  ...
  return (
    <>
      ...
      <div style={{ padding: "10px 20px 16px", display: "flex", flexDirection: "column", alignItems: "center", gap: "10px" }}>
        {/* Theme toggle */}
        <ThemeToggle />
  ...
  return (
    <>
      ...
      <motion.div variants={listAnim} initial="initial" animate="animate"
        style={{ position: "relative", paddingLeft: "16px", marginBottom: "32px" }}
      >
        <motion.div
          initial={{ scaleY: 0 }} animate={{ scaleY: 1 }}
          transition={{ duration: 0.6, delay: 0.1, ease }}
          style={{ position: "absolute", left: 0, top: "4px", bottom: "4px", width: "2px", borderRadius: "2px", background: `linear-gradient(to bottom, ${C.indigoDark}, ${C.indigo}33)`, transformOrigin: "top" }}
        />
        {education.map((edu, i) => (
          <motion.div
      ...
      >
        <motion.div
          ...
          style={{ position: "absolute", left: 0, top: "4px", bottom: "4px", width: "2px", borderRadius: "2px", background: `linear-gradient(to bottom, ${C.mintDark}, ${C.mint}33)`, transformOrigin: "top" }}
        />
...
function SkillsPage({ skillGroups }: { skillGroups: typeof SKILL_CATEGORIES }) {
  ...
  const grouped: Record<SkillTab, typeof SKILL_CATEGORIES> = {
    ...
    others:   skillGroups.filter((c) => c.key !== "backend" && c.key !== "frontend"),
  };
          ...
          {activeGroups.map((cat) => (
            <div key={cat.key} style={{ marginBottom: "20px" }}>
              {activeGroups.length > 1 && (
                <p style={{ fontSize: "10px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: C.textMuted, margin: "0 0 12px" }}>
                  {cat.label}
                </p>
              )}
              <motion.div
    ...
    <>
      ...
      {TESTIMONIALS.length === 0 ? (
        <motion.div
          ...
          style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "60px 20px", borderRadius: "16px", border: `1.5px dashed ${C.border}`, backgroundColor: C.card, gap: "12px", textAlign: "center" }}
        >
          <motion.div
            animate={{ rotate: [0, 5, -5, 0] }}
            transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
            style={{ width: "48px", height: "48px", borderRadius: "12px", backgroundColor: C.indigoLight, display: "flex", alignItems: "center", justifyContent: "center" }}
          >
            <Quote size={22} style={{ color: C.indigoDark }} />
          </motion.div>
          <p style={{ fontWeight: 700, fontSize: "14px", color: C.textDark, margin: 0 }}>No testimonials yet</p>
          <p style={{ fontSize: "12px", color: C.textMuted, margin: 0, maxWidth: "260px", lineHeight: 1.6 }}>
        ...
        >
          {TESTIMONIALS.map((t, i) => (
            <motion.div key={i} variants={itemAnim}
              whileHover={{ y: -3, boxShadow: `0 8px 28px rgba(225,29,72,0.12)` }}
              style={{ padding: "18px", borderRadius: "14px", border: `1px solid ${C.border}`, backgroundColor: C.card, display: "flex", flexDirection: "column", gap: "12px" }}
            >
              <div style={{ display: "flex", gap: "2px" }}>
                {Array.from({ length: 5 }).map((_, s) => (
                  <svg key={s} width="11" height="11" viewBox="0 0 24 24" fill={s < Math.floor(t.rating) ? C.amber : C.border} stroke="none">
                    <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
                  </svg>
                ))}
              </div>
              <div style={{ position: "relative" }}>
                <Quote size={16} style={{ color: C.indigoLight, position: "absolute", top: 0, left: 0 }} />
                <p style={{ fontSize: "12px", lineHeight: 1.7, color: C.textMid, margin: 0, paddingLeft: "20px" }}>{t.text}</p>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: "10px", paddingTop: "4px", borderTop: `1px solid ${C.border}` }}>
                <div style={{ width: "32px", height: "32px", borderRadius: "50%", background: `linear-gradient(135deg, ${C.indigoDark}, ${C.indigo})`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  <span style={{ fontSize: "10px", fontWeight: 700, color: "#fff" }}>{t.avatar}</span>
                </div>
                <div>
                  <p style={{ fontSize: "12px", fontWeight: 700, color: C.textDark, margin: 0 }}>{t.name}</p>
                  <p style={{ fontSize: "10px", color: C.textMuted, margin: "1px 0 0" }}>{t.role} · {t.company}</p>
                </div>
                <span style={{ marginLeft: "auto", fontSize: "9px", padding: "2px 8px", borderRadius: "99px", fontWeight: 600, backgroundColor: C.mintLight, color: C.mintDark }}>
                  {t.relationship}
                </span>
              </div>
            </motion.div>
...
function AnimatedBar({ pct, accent, delay = 0 }: { pct: number; accent: string; delay?: number }) {
  const C = useC();
  return (
    <div style={{ height: "6px", borderRadius: "99px", backgroundColor: C.border, overflow: "hidden" }}>
      <motion.div
        initial={{ width: 0 }}
        animate={{ width: `${pct}%` }}
    ...
    <>
      ...
      >
        {LANGUAGES.map((l, i) => {
          ...
          return (
            <motion.div
              key={l.name} variants={itemAnim}
              whileHover={{ x: 3 }}
              style={{ padding: "16px 18px", borderRadius: "14px", border: `1px solid ${C.border}`, backgroundColor: C.card }}
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "10px" }}>
                <div>
                  <p style={{ fontSize: "13px", fontWeight: 700, color: C.textDark, margin: 0 }}>{l.name}</p>
                  <p style={{ fontSize: "11px", color: C.textMuted, margin: "2px 0 0" }}>{l.nativeName}</p>
                </div>
                <span style={{ fontSize: "10px", padding: "3px 10px", borderRadius: "99px", fontWeight: 600, backgroundColor: C.indigoLight, color: C.indigoDark }}>
                  {l.level}
                </span>
              </div>
              <AnimatedBar
              ...
              />
              <p style={{ fontSize: "11px", color: C.textMuted, margin: "8px 0 0" }}>{l.note}</p>
            </motion.div>
          );
        })}
      </motion.div>
      ...
      >
        {BOOKS.map((b, i) => {
          ...
          return (
            <motion.div
              key={i} variants={itemAnim}
              whileHover={{ y: -4, boxShadow: `0 10px 30px ${tc.bg.replace("0.08", "0.25")}` }}
              style={{ padding: "16px", borderRadius: "14px", border: `1px solid ${C.border}`, backgroundColor: C.card, display: "flex", flexDirection: "column", gap: "10px" }}
            >
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "8px" }}>
                <motion.div
                  whileHover={{ rotate: 8 }}
                  style={{ width: "36px", height: "36px", borderRadius: "10px", backgroundColor: tc.bg, display: "flex", alignItems: "center", justifyContent: "center", border: `1px solid ${tc.border}`, flexShrink: 0 }}
                >
                  <BookOpen size={16} style={{ color: tc.color }} />
                </motion.div>
                <span style={{ fontSize: "9px", fontWeight: 600, padding: "3px 8px", borderRadius: "99px", backgroundColor: tc.bg, color: tc.color, border: `1px solid ${tc.border}`, whiteSpace: "nowrap" }}>
                  {b.theme}
                </span>
              </div>
              <div>
                <p style={{ fontSize: "12px", fontWeight: 700, color: C.textDark, margin: 0, lineHeight: 1.4 }}>{b.title}</p>
                <p style={{ fontSize: "11px", color: C.textMuted, margin: "2px 0 8px" }}>{b.author}</p>
                <p style={{ fontSize: "11px", color: C.textMid, lineHeight: 1.6, margin: 0, fontStyle: "italic", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                  "{b.quote}"
                </p>
              </div>
              <p style={{ fontSize: "11px", color: C.textMuted, lineHeight: 1.5, margin: 0, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                {b.reflection}
              </p>
            </motion.div>
          );
    ...
    <>
      ...
      >
        {[
          ...
          { Icon: Github,   label: "GitHub",   value: "github.com/javiergenepaul",     href: GITHUB_URL                },
          { Icon: Linkedin, label: "LinkedIn", value: "linkedin/gene-paul-mar-javier", href: LINKED_IN_URL             },
          { Icon: MapPin,   label: "Location", value: "Cebu, Philippines",             href: undefined                 },
        ].map(({ Icon, label, value, href }) => (
          <motion.div key={label} variants={itemAnim}
            whileHover={{ x: 4, boxShadow: `0 4px 20px rgba(225,29,72,0.1)` }}
            style={{ display: "flex", alignItems: "center", gap: "14px", padding: "14px 16px", borderRadius: "12px", backgroundColor: C.card, border: `1px solid ${C.border}` }}
          >
            <motion.div
              whileHover={{ scale: 1.1, backgroundColor: C.indigoDark }}
              style={{ width: "36px", height: "36px", borderRadius: "10px", flexShrink: 0, backgroundColor: C.indigoLight, display: "flex", alignItems: "center", justifyContent: "center" }}
            >
              <Icon size={15} style={{ color: C.indigoDark }} />
            </motion.div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <p style={{ fontSize: "10px", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em", color: C.textMuted, margin: 0 }}>{label}</p>
              {href ? (
                <a href={href} target={href.startsWith("http") ? "_blank" : undefined} rel="noopener noreferrer"
                  style={{ fontSize: "12px", fontWeight: 600, color: C.indigoDark, textDecoration: "none", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", display: "block" }}>
                  {value}
                </a>
              ) : (
                <p style={{ fontSize: "12px", fontWeight: 600, color: C.textDark, margin: 0 }}>{value}</p>
              )}
            </div>
          </motion.div>
        ))}
      ...
      <motion.a
        ...
        whileHover={{ scale: 1.02, boxShadow: `0 12px 32px rgba(225,29,72,0.5)` }}
        whileTap={{ scale: 0.98 }}
        style={{
...
function Separator() {
  const C = useC();
  return (
    <div style={{ margin: "0 20px", height: "1px", background: `linear-gradient(to right, ${C.indigoDark}44, ${C.indigo}44)` }} />
  );
}
